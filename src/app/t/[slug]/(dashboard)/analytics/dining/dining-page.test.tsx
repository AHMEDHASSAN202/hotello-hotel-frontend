...
describe('AnalyticsDiningPage', () => {
  ...
  it('the primary Export button calls apiBlob against the dining export endpoint (Task F3)', async () => {
    apiMock.api.mockResolvedValue(FIXTURE);
    apiMock.apiBlob.mockResolvedValue({ blob: new Blob(['x']), filename: 'sunrise-dining.xlsx' });
    renderPage();
    await waitFor(() => expect(apiMock.api).toHaveBeenCalled());
    fireEvent.click(screen.getByRole('button', { name: en.reports.export }));
    await waitFor(() =>
      expect(apiMock.apiBlob).toHaveBeenCalledWith('/tenant/reports/dining/export?preset=last7'),
    );
  });

  it('the secondary "export raw data" button calls apiBlob against the orders-feed export endpoint (Task F3)', async () => {
    apiMock.api.mockResolvedValue(FIXTURE);
    apiMock.apiBlob.mockResolvedValue({ blob: new Blob(['x']), filename: 'sunrise-orders-feed.csv' });
    renderPage();
    await waitFor(() => expect(apiMock.api).toHaveBeenCalled());
    fireEvent.click(screen.getByRole('button', { name: en.reports.exportRawData }));
    await waitFor(() =>
      expect(apiMock.apiBlob).toHaveBeenCalledWith('/tenant/reports/orders-feed/export?preset=last7'),
    );
  });
});
